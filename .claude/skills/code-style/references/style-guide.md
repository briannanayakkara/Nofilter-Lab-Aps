...
- **Shopify**: all Storefront API access goes through `lib/shopify.js` (a direct `fetch` to the Storefront GraphQL API — no SDK). Never call the Storefront API directly from a component. Check `isShopifyConfigured()` before any flow that requires a real store connection. Checkout uses the Cart API (`cartCreate` → `checkoutUrl`); the old Buy Button SDK / Checkout API was removed by Shopify in Storefront API 2025-04.
